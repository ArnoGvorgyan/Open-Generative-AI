...
export default function ApiKeyModal({ onSave }) {
  ...
  return (
    <div className="min-h-screen bg-[#030303] flex items-center justify-center px-4 font-inter">
      <div className="w-full max-w-sm bg-[#0a0a0a]/40 backdrop-blur-xl border border-white/10 rounded-xl p-6 sm:p-10 shadow-2xl">
        <div className="flex flex-col items-center text-center mb-6 sm:mb-10">
          <img src="/logo.png" alt="Araqs" className="w-14 h-14 rounded-2xl object-cover mb-6" />
          <h1 className="text-xl font-bold text-white tracking-tight mb-2">
            Araqs
          </h1>
          <p className="text-white/40 text-[13px] leading-relaxed px-4">
            Enter your <a href="https://araqs.ai" target="_blank" rel="noreferrer" className="text-[#d9ff00] hover:text-[#e5ff33] transition-colors">Araqs</a> API key to start creating
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <p className="text-center text-[12px] text-white/20 pt-2">
            Need a key?{' '}
            <a href="https://araqs.ai" target="_blank" rel="noreferrer" className="text-white/40 hover:text-[#d9ff00] transition-colors font-medium">
              Get one free →
            </a>
          </p>
        </form>
